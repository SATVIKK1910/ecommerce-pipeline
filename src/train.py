...
def evaluate(name, model, X_tr, X_te, y_tr, y_te):
    ...
    print(f"Model: {name} | ROC-AUC: {auc:.4f} | F1: {f1:.4f}")
    print(f"TN={cm[0,0]} FP={cm[0,1]} FN={cm[1,0]} TP={cm[1,1]}")
    return model, auc, f1

lr, lr_auc, lr_f1 = evaluate("Logistic Regression", LogisticRegression(random_state=42, max_iter=1000), X_train_sc, X_test_sc, y_train, y_test)
rf, rf_auc, rf_f1 = evaluate("Random Forest", RandomForestClassifier(n_estimators=100, random_state=42), X_train, X_test, y_train, y_test)
