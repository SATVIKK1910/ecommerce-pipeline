...
import pandas as pd
X_new = pd.DataFrame([{
    "customer_total_spend_90d": 1500.0,
    "customer_order_count_90d": 3,
    "avg_basket_size": 500.0,
    "distinct_categories_bought": 4,
    "days_since_last_order": 15,
    "total_orders_alltime": 8
}])
X_scaled = scaler.transform(X_new[features])
pred  = model.predict(X_scaled)
proba = model.predict_proba(X_scaled)[:, 1]
print(f"Prediction: {pred[0]} | Probability: {proba[0]:.2%}")
```
